            ...
            <div>
              <img src="/images/pharmarxlogo.png" alt='pharmarx' className="h-30 w-28 mx-auto mb-10" />
              {
                upper_panel.map((link: (string | JSX.Element)[], id: number) =>
                  <a href={`${link[1]}`}
                    key={`link-${id}`}
                    className="text-[#808191] text-base bg-[#d9e0e7] flex items-center my-3 rounded-2xl flex items-start p-3 no-underline gap-2 hover:bg-[#3f6387] hover:text-[#fff] active:bg-[#3f6387]"
                  >
                    {link[2]}
                    {link[0]}
                  </a>
